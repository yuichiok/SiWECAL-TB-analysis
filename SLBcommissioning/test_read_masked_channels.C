...
void test_read_masked_channels(TString filename = "15102021/Run_Settings_DataTaking_TB2021_15102021", bool debug = true)
{
  ...
  gStyle->SetOptStat(0);
  gStyle->SetPalette(kInvertedDarkBodyRadiator); // Cherry);
  mask_chip_chn[13]->SetTitle("Layer 13 Mask cells Chip vs. Channel");
  mask_chip_chn[13]->GetXaxis()->SetTitle("CHIP");
  mask_chip_chn[13]->GetYaxis()->SetTitle("CHANNEL");
  gPad->SetLeftMargin(0.12);
  gPad->SetRightMargin(0.09);
  mask_chip_chn[13]->Draw("col");
  canvas_chn_chip->Print("plots/mask_ChipChn_slab1.pdf");

  TCanvas *canvas_x_y = new TCanvas("canvas_x_y", "canvas_x_y", 800, 800);
  gStyle->SetOptStat(0);
  gStyle->SetPalette(kInvertedDarkBodyRadiator); // Cherry);
  mask_x_y[13]->SetTitle("Layer 13 Mask cells XY positions");
  mask_x_y[13]->GetXaxis()->SetTitle("x [mm]");
  mask_x_y[13]->GetYaxis()->SetTitle("y [mm]");
  gPad->SetLeftMargin(0.12);
  gPad->SetRightMargin(0.09);
  mask_x_y[13]->Draw("col");
