...
int number_occurencies(TString run="32001") {
  
  

  TFile *file_histo = new TFile("run_"+run+"_histograms.root", "read");
  cout<<"run_"+run+"_histograms.root"<<endl;
  TFile *file_writing = new TFile("run_"+run+"_coincidences_studies.root", "NEW");
  ...
  for (int ref=5; ref<9; ref++ ) {
    ...
    for(Int_t chip=0; chip<NCHIP; chip++) {
      ...
      canvas[chip]=new TCanvas(TString::Format("slab_%i_chip_%i_number_occur", ref,chip),TString::Format("slab_%i_chip%i_number_occur", ref,chip), 1800,1400);
	...
	cout<<TString::Format("number_coincidences_slab_%i_chip_%i_slab_%i_chip_%i", ref,chip, slab, chip0)<<endl;
	...
	// coincidences[chip];
      ...
      g_coinc[chip]->SetTitle(TString::Format("number of coincidences with slab %i",ref));
      g_coinc[chip]->Draw("alp");


      g_coinc[chip]->SetName(TString::Format("slab_%i_chip_%i_number_occur", ref,chip));
