...
void CreateHistos(TString run="3GeVMIPscan", int igain=0, int isca=0, int run0=43, int runN=124){
  ...
  //for (int isca=0; isca<2; isca++) {
  runN=run0+1;  
  for (int iname=run0; iname<runN; iname ++) {
    
    //  for(int i=0; i<2; i++) {
    TString name=TString::Format("%s_run_0500%i_%s",run.Data(),iname,gain[igain].Data());//.root3GeVMIPscan_run_050043_highgain";
    if(iname>99) name=TString::Format("%s_run_050%i_%s",run.Data(),iname,gain[igain].Data());
      
    TString newfile=TString::Format("Summary_%s.root",name.Data());
    if(isca>0)  newfile=TString::Format("Summary_%s_sca%i.root",name.Data(),isca-1);

    if(isca>0 && iname>99) name=TString::Format("%s_run_050%i_%s_sca%i",run.Data(),iname,gain[igain].Data(),isca-1);//TString::Format("sca0/%s_sca0",name.Data());
    if(isca>0 && iname<100) name=TString::Format("%s_run_0500%i_%s_sca%i",run.Data(),iname,gain[igain].Data(),isca-1);//TString::Format("sca0/%s_sca0",name.Data());
...
void NoiseStudy2(TString run="3GeVMIPscan"){

  //CreateHistos(run,igain,isca,run0,runN);
