...
int MIP_LS_angle(){
  ...
  for(int iasu=8; iasu<9; iasu++) {
    ...
    for(int ichn=0; ichn<16; ichn++) {
          ...
          double x[10], y[10], ey[10], entries[10];

  
      for(int iangle=0; iangle<max_angle; iangle++) {
      // int asu = 8; // asu number from 1 (closest to the dif) to 8.
      
      int angle_int=0;
      if(iangle==0) angle_int=0;
      if(iangle==1) angle_int=10;
      if(iangle==2) angle_int=20;
      if(iangle==3) angle_int=30;
      if(iangle==4) angle_int=40;
      if(iangle==5) angle_int=45;
      ...
      angle=TString::Format("angle%i",angle_int); 
	...
	if(hmip_good ) {
	  ...
	  entries[iangle]=hmip_good->GetEntries();
	  
	  if(hmip_good->GetEntries()>50) {
	    y[iangle]=fitsnr_temp->GetParameter(1);
	    ey[iangle]=fitsnr_temp->GetParameter(0);
	  }

	}
	position_channel[ichn]=new TGraphErrors(max_angle,x,y,0,ey);
	width_channel[ichn]=new TGraphErrors(max_angle,x,ey,0,0);
	entries_channel[ichn]=new TGraphErrors(max_angle,x,entries,0,0);
    ...
    TCanvas *canvas = new TCanvas(TString::Format("canvas_asu%i",iasu),TString::Format("canvas_asu%i",iasu),1400,600);
    canvas->Divide(3,1);
    canvas->cd(1);
    for(int ichn=0;ichn<16;ichn++) {
      if(ichn<8) {
	...
	position_channel[ichn]->SetLineWidth(2);
      ...
      if(ichn==0) {
	position_channel[ichn]->SetTitle(TString::Format("ASU%i",iasu));
	position_channel[ichn]->GetXaxis()->SetTitle("angle in degrees");
	position_channel[ichn]->GetYaxis()->SetTitle("MPV [ADC]");
	position_channel[ichn]->GetYaxis()->SetRangeUser(0,200);
	...
	width_channel[ichn]->SetLineWidth(2);
      ...
      if(ichn==0) {
	width_channel[ichn]->SetTitle(TString::Format("ASU%i",iasu));
	width_channel[ichn]->GetXaxis()->SetTitle("angle in degrees");
	...
	entries_channel[ichn]->SetLineWidth(2);
      ...
      if(ichn==0) {
	entries_channel[ichn]->SetTitle(TString::Format("ASU%i",iasu));
	entries_channel[ichn]->GetXaxis()->SetTitle("angle in degrees");
	entries_channel[ichn]->GetYaxis()->SetTitle("Ntriggers");
	//entries_channel[ichn]->GetYaxis()->SetRangeUser(0,20);
	entries_channel[ichn]->Draw("alp");
      } else entries_channel[ichn]->Draw("lp");
      
    }//ichn

    canvas->Print(TString::Format("mip_canvas_asu%i_%s.png",iasu,angle.Data()));
