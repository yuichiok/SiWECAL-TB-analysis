#ifndef RAW2ROOT_TDC_test_CC
#define RAW2ROOT_TDC_test_CC
...
// .x RAW2ROOT_TDC_test.C+

using std::cout;
using std::endl;

class RAW2ROOT_TDC_test {
...
void RAW2ROOT_TDC_test::Initialisation() {
  fout->cd(); R2Rstate=-1;
...
void RAW2ROOT_TDC_test::treeInit() { //init data for a single SPILL ?
...
//******************************************************************************************************************
void RAW2ROOT_TDC_test::searchNmax(int N, int Size, Float_t * table, Float_t * tableout , int * ranks) {
...
void RAW2ROOT_TDC_test::analyse_dataintegrity() {
...
void RAW2ROOT_TDC_test::analyse_hits() {
...
void RAW2ROOT_TDC_test::plotHistos() {
...
void RAW2ROOT_TDC_test::printEvent(std::vector < unsigned short int > & eventData) {
  if (_debug) {
...
int RAW2ROOT_TDC_test::readEvent(std::vector < unsigned short int > & eventData, int bcidthres) {
  ...
  for (int k=0; k<NCHIP; k++) {
	...
	for (int ichan=0; ichan<NCHANNELS; ichan++) {
	  if(gain_hit_low[k][ibc][ichan]%2==1){
	...
	for (int ichan=0; ichan<NCHANNELS; ichan++) {
	  if  (charge_low[k][ibc][ichan] < NEGDATA_THR) count_negdata++;
	}//ichan
...
int RAW2ROOT_TDC_test::data_integrity(std::vector < unsigned short int > & eventData, int i, int local_offset, int nColumns, int ichip){
  ...
  //create vectors of low/low gain hit bits for comparison 
...
void RAW2ROOT_TDC_test::ReadFile(TString inputFileName, bool overwrite, TString outFileName, int bcidthres, int maxevt) {
  ...
  ////////////////////////  GETTREE   ////////////////////////
  int RAW2ROOT_TDC_test::GetTree (TString rootfilename) { //from raw2root 1st pass
  ...
  ////////////////////////  BUILDTIMEDEVENTS   ////////////////////////
  int RAW2ROOT_TDC_test::BuildTimedEvents (TString rootfilename) {
    ...
    if (! GetTree(rootfilename)) {
      if(_savelog) out_log << "RAW2ROOT_TDC_test ERROR: "<< rootfilename.Data() << " not found" << endl;
      return 0;
    }
